...
description: Evaluate .ai-docs/ files for content quality and frontmatter accuracy. Checks if patterns/keywords match actual content, identifies missing patterns, flags domain mismatches, notes content gaps (patterns without examples or rationale), and verifies section summary accuracy.
tools: Read, Glob
model: sonnet
---
...
- Do the `keywords[]` supplement the patterns, or do they duplicate them without adding value?
...
4. Domain mismatch is HIGH severity; missing patterns are MEDIUM; missing examples are LOW
